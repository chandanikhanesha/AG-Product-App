...
import { styles } from './add_delay_dialog.styles';

class AddDelayDialog extends Component {
  state = {
    seedCompanyId: '',
    certainDays: 0,
    productCategories: [],
  ...
  componentWillMount = () => {
    const { delayProducts } = this.props;
    let selectedCompanyIds = [];
    let selectedSeedCompanyIds = [];
    delayProducts.forEach((delayProduct) => {
      selectedCompanyIds = [...selectedCompanyIds, ...delayProduct.companyIds];
      selectedSeedCompanyIds = [...selectedSeedCompanyIds, ...delayProduct.seedCompanyIds];
    });
  ...
  create = async () => {
    const { createDelayProduct, organizationId, loadDatas, onClose } = this.props;
    const { name, companyIds, seedCompanyIds, isFixed, fixedDate, certainDays } = this.state;
    if (name === '') {
    ...
    await createDelayProduct(createData);
    loadDatas();
    onClose();
  };

  saveAndAdd = async () => {
    const { createDelayProduct, organizationId } = this.props;
    const { name, companyIds, seedCompanyIds, isFixed, fixedDate, certainDays } = this.state;
    if (name === '') {
      this.setState({ nameAlertShow: true });
      return;
          ...
          <GridItem xs={12}>
            <React.Fragment>
              ...
              <div>
                ...
                />
                <span style={{ color: 'red' }}>{dateAlertShow && dateAlertText}</span>
              </div>
              <div>
                <FormControlLabel
                  //value="top"
...
export default withStyles(styles)(AddDelayDialog);
