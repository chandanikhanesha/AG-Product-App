...
import { editInterestChargeStyles } from './edit_interest_charge.styles';

class EditInterestCharge extends Component {
  state = {
    interestChargeId: null,
    seedCompanyId: '',
    certainDays: 0,
    compoundingDays: 0,
    interestCharge: 0,
  ...
  componentDidMount() {
    this.initInterestCharge();
  }

  initInterestCharge() {
    const { interestCharges, match } = this.props;

    const interestCharge = interestCharges.find((interestCharge) => `${interestCharge.id}` === match.params.id);

    if (interestCharge) {
      this.setState({
        interestChargeId: interestCharge.id,
        seedCompanyId: interestCharge.seedCompanyId,
        certainDays: interestCharge.certainDays,
        compoundingDays: interestCharge.compoundingDays,
        interestCharge: interestCharge.interestCharge,
        productCategories: interestCharge.productCategories,
        companyIds: interestCharge.companyIds,
        name: interestCharge.name,
        fixedDate: interestCharge.fixedDate,
        applyToWholeOrder: interestCharge.applyToWholeOrder,
        applyToFixedDate: interestCharge.applyToFixedDate,
        applyToCertainDate: interestCharge.applyToCertainDate,
        useByDefault: interestCharge.useByDefault,
      });
    }
  }

  updateCharge = (e) => {
    const { updateInterestCharge, organizationId: organizationId } = this.props;
    const {
      interestChargeId,
      seedCompanyId,
      certainDays,
    ...
    if (!name) return;
    if (!applyToFixedDate && !applyToCertainDate) return;
    ...
    updateInterestCharge(interestChargeId, data).then(() => this.props.history.push('/app/setting/interest_charge'));
  };
  ...
  render() {
    ...
    if (seedCompanyId && !seedCompany) return null;

    return (
      <div>
        <GridContainer justifyContent="center">
          <GridItem xs={12}>
            <form action="#" onSubmit={this.updateCharge}>
              <Card>
                <CardHeader>
                ...
                <CardFooter>
                  <CTABar
                    ...
                    secondaryAction={() => {
                      ...
                      this.props.history.push('/app/setting/interest_charge');
                    }}
                  />
                </CardFooter>
              </Card>
...
export default withStyles(editInterestChargeStyles)(EditInterestCharge);
