...
class ProductSelector extends Component {
  ...
  availableTypes(itemType, i) {
    ...
    if (seedType !== 'BUSINESS') available = available.filter((p) => p.seedType === seedType);
  ...
  addProduct = () => {
    ...
    const { addProductToOrder, classes } = this.props;
  ...
  onEditProductChange = () => {
    ...
    if (selectedProduct && `${quantity}` !== '0') {
      selectedProduct.orderQty = parseInt(quantity);
  ...
  showSeedSizeAndPackaging() {
    const { seedType, purchaseOrder } = this.props;
    return seedType !== 'BUSINESS' && purchaseOrder.isSimple === true;
  }

  render() {
    const { editMode, columns, selectedProduct, quantity, packagingId, seedSizeId } = this.state;
    const { classes, seedType, packagings, seedSizes, seedCompany } = this.props;
            ...
            <Paper className={classes.list}>
              ...
              <List dense={true}>
                <ListItem>
                  {selectedProduct && (
                    <div>
                      <div>MSRP: {numberToDollars(selectedProduct.msrp || selectedProduct.costUnit)}</div>
                      <div>
                        <CustomInput
                          labelText="Quantity"
                          id="quantity"
