...
import { createFarm } from '../../store/actions';
...
class CreateShareholder extends Component {
  ...
  onSubmit = (e) => {
    ...
    const { createFarm, match } = this.props;
    const { name } = this.state;
    const customerId = match.params.customer_id;
    createFarm(customerId, { name }).then(() => {
      this.props.history.push(`/app/customers/${customerId}#farms`);
    });
  };
